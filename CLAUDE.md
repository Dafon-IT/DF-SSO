...
> **部署狀態：** 目前僅部署於 **Test 環境**（`*-test.apps.zerozero.tw`）。`docker-compose-prod.yml` 為未來正式環境的樣板，尚未啟用；任何 `.md` 或 env 檔中的「prod」通常指「未來 prod」而非目前已上線。
...
docker-compose-test.yml     # ★ 目前 Test 實際部署用
docker-compose-prod.yml     # 未來 prod 樣板（未啟用）
```
...
- [migrations/](backend/migrations/) — `node-pg-migrate`，檔名時間戳編號
...
- Seed 相關 migration 要 idempotent
...
cd backend
npm run dev              # nodemon
npm run migrate:up       # 套用所有 migration
npm run migrate:down     # 回退一步（注意管理 domain 保留規則）
...
# dev / prod compose 為樣板，本機與未來正式環境使用，尚未啟用
```
